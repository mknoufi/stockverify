        ...
        <SafeAreaView className="flex-1">
          <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
            <View className="px-6 pt-4">
              ...
              >
                {item.itemCode && (
                  <DetailRow icon={Hash} label="Item Code" value={item.itemCode} />
                )}
                <DetailRow icon={Barcode} label="Barcode" value={item.barcode} />
                {item.serialBarcode && (
                  <DetailRow icon={Barcode} label="Serial Barcode" value={item.serialBarcode} />
                )}
                {item.category && (
                  <DetailRow icon={Folder} label="Category" value={item.category} />
                )}
                {item.subCategory && (
                  <DetailRow icon={Folder} label="Sub-Category" value={item.subCategory} />
                )}
                {item.brand && (
                  <DetailRow icon={Building2} label="Brand" value={item.brand} />
                )}
                <DetailRow icon={Box} label="Unit of Measure" value={item.uom} />
                <DetailRow
                  icon={Box}
                  label="ERP Stock Quantity"
                  value={`${item.systemStock} ${item.uom}`}
                  valueColor="text-blue-400"
                />
                <DetailRow icon={DollarSign} label="MRP" value={`₹${item.mrp.toLocaleString()}`} />
                <DetailRow
                  icon={DollarSign}
                  label="Sale Price"
                ...
                {item.taxClassification && (
                  <>
                    {item.taxClassification.gstPercent && (
                      <DetailRow
                        icon={FileText}
                        label="GST %"
                        value={`${item.taxClassification.gstPercent}%`}
                      />
                    )}
                    {item.taxClassification.hsn && (
                      <DetailRow icon={FileText} label="HSN Code" value={item.taxClassification.hsn} />
                    )}
                  </>
                )}
                {item.lastVerifiedDate && (
                  <DetailRow
                    ...
                    value={new Date(item.lastVerifiedDate).toLocaleDateString()}
                  />
                )}
                {item.lastVerifiedBy && (
                  <DetailRow icon={User} label="Last Verified By" value={item.lastVerifiedBy} />
                )}
              </Animated.View>
